...
face_fold_ids=os.listdir(data_path)[0:10]
...
#Train Batches
train_batch_size=int(np.ceil(len(t_image_list)/BATCH_SIZE))
val_batch_size=int(np.ceil(len(v_image_list)/BATCH_SIZE))
print('Total Train Batches: {}'.format(train_batch_size))
print('Total Val Batches: {}'.format(val_batch_size))
        
train_loss_list,val_loss_list=[],[]
train_roc_score,val_roc_score=[],[]
...
with tf.Session() as sess:
    ...
    for epoch in range(EPOCHS):
        ...
        for ix in tqdm(range(train_batch_size)):
            ...
            t_loss,p_train,_=sess.run([loss,out,train_opt],feed_dict=input_dict)
            tmp_train_preds.extend(p_train)
        train_roc=roc_auc_score(t_labels,tmp_train_preds)
        ...
        for ix in tqdm(range(val_batch_size)):
            ...
            v_loss,p_val=sess.run([loss,out],feed_dict=input_dict)
            tmp_val_preds.extend(p_val)
        val_roc=roc_auc_score(v_labels,tmp_val_preds)
        
            
            
        print('Training Loss: {:.4f} and ROC Score: {:.4f}'.format(t_loss,train_roc))
        print('Validation Loss: {:.4f} and ROC Score: {:.4f}'.format(v_loss,val_roc))
        train_loss_list.append(t_loss)
        val_loss_list.append(v_loss)
        train_roc_score.append(train_roc)
        val_roc_score.append(val_roc)
        
        #####Checkpoint
        if epoch==0: #Save first Checkpoint
            saver.save(sess,os.path.join(ckpt_path,'model.ckpt'))
            print('Validation Loss improved from 0 to {:.4f}'.format(v_loss))
        elif v_loss<=np.min(val_loss_list):
            saver.save(sess,os.path.join(ckpt_path,'model.ckpt'))
            print('Validation Loss improved from {:.4f} to {:.4f}'.format(tmp_v_loss,v_loss))
        else:
...
plt.legend()
plt.figure()
plt.plot(epochs,train_roc_score,'b',color='red',label='Training ROC')
plt.plot(epochs,val_roc_score,'b',color='blue',label='Validation ROC')
plt.title('Training and Validation ROC score')
plt.legend()
plt.figure()
plt.show()
...
####################################################################################
#Identifying person
#Select one random image from unseen test data
rand_ix=np.random.randint(0,len(test_image_list))
rand_img=test_image_list[rand_ix][0] #Take either left or right image because we only need a face

test_recog_list=[]
for i in face_fold_ids:
    fold_ix=os.listdir(os.path.join(data_path,i))
    rand_img_ix=np.random.choice(fold_ix)
    img_ix=os.path.join(data_path,i,rand_img_ix)
    test_recog_list.append([rand_img,img_ix])

tmp_preds=[]   
with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    saver.restore(sess,os.path.join(ckpt_path,'model.ckpt'))
    for img_ix in tqdm(test_recog_list):
        image_left,image_right=get_test_images(img_ix)
        tmp_preds.extend(sess.run(out,feed_dict={inp_1:image_left,inp_2:image_right}))

predicted_index=tmp_preds.index(np.max(tmp_preds))
#Plot left image and predicted similar right image
row,col=1,2
text=['Input Image','Similar Predicted Image']
fig=plt.figure(figsize=(10,10))
for i in range(1,row*col+1):
    img=cv2.imread(test_recog_list[predicted_index][i-1])
